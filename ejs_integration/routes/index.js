...
router.get("/",ensureAuthenticated,async (req, res) => {
  console.log(req.user);
    let ans;
    try {
      const status = await axios.get("http://localhost:3000/api/get_market_status");
    ...
    newsModel.find({}, (err, items) => {
      if (err) {
        ...
        res.render("index", { title: "News Feed", nifty: ans, items: items,user: req.user });
      }
    });
  });
  ...
  router.get("/community", ensureAuthenticated, async (req, res) => {
    ...
    res.render("community", { title: "Community Feed", posts: posts.data.posts,user: req.user });
  });
  ...
  router.get("/volatility", ensureAuthenticated, (req, res) => {
    res.render("volatility", { title: "Risk to Volatile Calculator",user: req.user });
  });
